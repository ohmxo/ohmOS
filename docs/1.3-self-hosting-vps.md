# Self-hosting ohmOS on a VPS or Coolify

ohmOS runs as a **single Bun production server**. That server handles:
...
- `STORAGE_PROVIDER` — optional explicit storage backend (`s3`); auto-detected if unset
...
- `IP_GEOLOCATION_URL_TEMPLATE` — optional override for the IP-geolocation provider (defaults to `https://ipwho.is/{ip}`); use `{ip}` as the placeholder
...
ohmOS supports **two Redis modes**.
...
Email-based account recovery uses Resend. Set both variables together; if either one is missing, ohmOS falls back to Telegram-linked account recovery when available.

```bash
RESEND_API_KEY="..."
RECOVERY_EMAIL_FROM="ohmOS <noreply@example.com>"
```
...
ohmOS supports **two realtime modes**.
...
ohmOS uses **S3-compatible object storage** for cloud sync and private AI chat images. <!-- pragma: allowlist secret -->

Works with MinIO, Cloudflare R2, Backblaze B2, AWS S3, or any S3-compatible provider: <!-- pragma: allowlist secret -->
...
2. The build script reads `SOURCE_COMMIT` (Coolify) or `GIT_COMMIT_SHA` (generic CI)
...
```ini
[Unit]
Description=ohmOS self-hosted server
...
- `/api/sync/*` endpoints use the storage adapter (`api/_utils/storage.ts`), which targets S3-compatible storage. See section 6 for configuration. <!-- pragma: allowlist secret -->
...
then the issue is usually the host's Docker bridge / iptables setup, not ohmOS itself.
...
After applying it, re-test Redis TCP between containers before debugging ohmOS.
...
configuration there rather than changing ohmOS application code.

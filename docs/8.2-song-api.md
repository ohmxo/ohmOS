...
The Song API provides endpoints for managing a music library in ohmOS, including CRUD operations for songs, lyrics fetching from KuGou, AI-powered translations, Japanese furigana annotations, and "soramimi" (空耳) phonetic readings.

Refactored song routes use the shared `apiHandler` pattern with optional auth plus action-specific permission checks.
Public metadata reads and first-time lyrics, translation, furigana, and
soramimi generation do not require login. Optional routes treat partial,
stale, or invalid credentials as anonymous; authenticated ownership checks
still apply to metadata mutations and protected force-refresh operations.
...
- **Content** (heavy ~5-50KB): raw KuGou lyrics, processed `zh-TW`/`zh-CN` lyric variants, translations, furigana, and soramimi annotations
...
|--------|------|-------------|
| GET | `/api/songs` | List songs with optional filtering |
| POST | `/api/songs` | Create song or bulk import |
  ...
  "songs": [
    {
      ...
      "lyrics": {
        ...
        "parsedLinesByLanguage": {
          "zh-TW": [{ "startTimeMs": "0", "words": "頭髮" }],
          "zh-CN": [{ "startTimeMs": "0", "words": "头发" }]
        }
      },
      "translations": { "zh-TW": "...", "zh-CN": "..." },
...
|-----------|------|-------------|
| `include` | string | Comma-separated: `metadata`, `lyrics`, `translations`, `furigana`, `soramimi` (default: `metadata`) |
| `lyricsLanguage` | string | Processed primary lyric script: `zh-TW` or `zh-CN` (default: `zh-TW`) |
...
| `translate` | Generate full translation (non-streaming JSON) | First: No, Force: Yes |
...
| `lyricsLanguage` | string | Primary lyric script: `zh-TW` or `zh-CN` (default: `zh-TW`) |
...
- Raw LRC/KRC from KuGou remains stored in Simplified Chinese
- Both processed `zh-CN` and OpenCC-converted `zh-TW` lines (including KRC word timings) are stored with the song
- `lyricsLanguage` selects which processed script is returned as `parsedLines`
- iPod and Karaoke expose Automatic, Traditional, and Simplified choices in the Pronunciation menu; Automatic follows the current Chinese ohmOS locale and otherwise preserves the Traditional default
- Japanese lyrics are detected and preserved without conversion

### Embedded Translations

KRC files may contain embedded Chinese translations in a base64-encoded `[language:]` tag. The API extracts the Simplified source, stores both `translations["zh-CN"]` and its Traditional `translations["zh-TW"]` conversion, and bypasses AI generation for either Chinese target. AI-generated Chinese translations are also saved under both script keys.

...
**Implementers:** see [Theme architecture (implementation)](/docs/theme-architecture) for `data-os-platform`, CSS layers, menu tokens, and Tailwind variants.

ohmOS supports 4 base OS themes emulating classic operating systems, plus macOS Aqua variants for material (`classic` / `glass`) and color scheme (`light` / `dark`). Themes use comprehensive CSS variable systems, centralized metadata helpers, and platform-specific UI behaviors.

## Available Themes

| Theme ID | Name | Platform | Era | Key Elements |
|----------|------|----------|-----|--------------|
| `macosx` | Aqua | Mac OS X | ~2001 | Glossy buttons, traffic lights, pinstripe, dock; supports **material** (`classic` / `glass`) and **color scheme** (`light` / `dark`) |
...
TypeScript theme definitions intentionally store metadata and wallpaper defaults only. Runtime visual values live in `src/styles/themes/tokens.css` and are consumed through `--os-*` CSS variables and Tailwind `os-*` utilities.
...
## Accent Colors (Aqua & System 7)

Aqua (`macosx`) and System 7 (`system7`) support user-selectable **accent colors** that drive the active selection gradient across menus, dropdowns, buttons, sidebar highlights, switches, and focus rings. Accents are applied as inline CSS custom properties on `<html>` (`--os-color-selection-*` and derived accent vars), so any surface that reads those tokens updates automatically.

| Accent ID | Description |
|-----------|-------------|
| `wallpaper` | **Default.** Samples a dominant color from the active desktop wallpaper (live-updated). |
| `default` | Labelled **System** in Control Panels — clears overrides so the theme's classic selection color (Aqua blue / System 7 black) remains the source of truth. |
| Named swatches | `graphite`, `blue`, `purple`, `pink`, `red`, `orange`, `yellow`, `green`, `teal` — fixed palette colors with per-chrome tuned gradients. |

Accent choice is persisted in display settings and exposed in **Control Panels → Appearance** when the current theme is Aqua or System 7. Windows themes (XP, 98) do not offer accent selection.
...
export const useThemeStore = create<ThemeState>((set) => ({
  ...
  hydrate: () => {
    const saved = localStorage.getItem("ryos:theme") as OsThemeId | null;

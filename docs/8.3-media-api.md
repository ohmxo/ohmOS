...
The Media API provides endpoints for text-to-speech synthesis, audio transcription, and YouTube music search. These endpoints power the audio-related features in ohmOS applications like iPod, Karaoke, and voice input.

All endpoints run on the standalone Bun server and implement rate limiting to prevent abuse.
...
Resolves Apple Music catalog artwork (and basic metadata) for an `am:` catalog song ID using Apple's public iTunes Lookup API (no Apple auth required). Apple Music songs cached in Redis often have no `cover` because client-side MusicKit artwork is never persisted server-side; this endpoint backfills real cover art (used by the admin UI). Results are cached in Redis.

### Endpoint

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/apple-music-artwork` | Resolve cover art + metadata for a catalog song |
...
|-----------|------|----------|-------------|
| `id` | string | Yes | Apple Music catalog ID in the form `am:<number>` |

Only numeric catalog IDs can be resolved. Library IDs (`i.<hash>`), stations (`station:…`), and playlists (`playlist:…`) are not catalog songs and return `400`.
...
| Field | Type | Description |
|-------|------|-------------|
| `cover` | string \| null | Cover URL with `{w}`/`{h}` size placeholders, or `null` when unavailable |
| `artist` | string? | Artist name |
| `title` | string? | Track name |
| `album` | string? | Collection/album name |
| `url` | string? | Track or collection view URL |

**Not Found (404):**

When no artwork can be resolved, the same shape is returned with `cover: null`.
...
```json
{
  "error": "Expected an Apple Music catalog id (am:<number>)"
}
```

### Caching

- Resolved artwork is cached for 30 days; "not found" results for 1 day.
- Successful responses send `Cache-Control: public, max-age=86400`; misses send `public, max-age=3600`.
...
| `REDIS_KV_REST_API_URL` | Rate Limiting, Artwork cache | Upstash Redis URL |
| `REDIS_KV_REST_API_TOKEN` | Rate Limiting, Artwork cache | Upstash Redis token |

...
ohmOS APIs are implemented as Node-style route handlers under `api/`, served by the standalone Bun API server, with shared `apiHandler` and `api/_utils` primitives providing consistent CORS, method routing, auth, and error handling patterns.
...
tools: {
  launchApp: {
    description: "Launch ohmOS application",
    parameters: z.object({
...
export const CORE_PRIORITY_INSTRUCTIONS = `
  You are Ryo, an AI assistant in ohmOS...
`;
...
- The **standalone Bun server** (`scripts/api-standalone-server.ts`) discovers and executes the `api/*` handlers via a request/response adapter.

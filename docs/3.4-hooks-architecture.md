...
ohmOS provides roughly 75 custom hooks organized by functionality, enabling audio playback, window management, media handling, undo/redo, realtime presence, background synchronization, and more.
...
Pointer-based long-press detection for mouse, pen, and touch call sites.

```typescript
interface UsePointerLongPressOptions {
  onLongPress: (event: PointerEvent) => void;
  delay?: number;
}

// Usage
const longPressHandlers = usePointerLongPress({
...
Mounts the Cloud Sync v2 engine (`src/sync/engine.ts`) while a logged-in user has Auto Sync enabled. The engine decomposes store state into per-key documents via namespace codecs (`src/sync/codecs.ts`), diffs them against a persisted shadow map to derive pending ops, and exchanges deltas with `/api/sync/v2/*`. The hook wires the triggers: realtime `sync-ops` events on `private-sync-{username}` (small remote changes apply with zero HTTP requests), visibility/focus/online cursor checks, a slow heartbeat poll, and explicit change/check events from app code. Used by `App.tsx` via the deferred runner.

...
Winamp brings the iconic Winamp media player to ohmOS, powered by [Webamp](https://github.com/captbaritone/webamp) by Jordan Eldredge. It plays music from your iPod library via YouTube, supports classic Winamp skins, and features the familiar equalizer, playlist editor, and transport controls.

## Overview

The Winamp app faithfully recreates the legendary Winamp 2.x experience inside your ohmOS desktop. Built on top of Webamp — a pixel-perfect HTML5 reimplementation of Winamp 2.9 — it renders the original three-panel layout: main window, equalizer, and playlist editor. Tracks are sourced from the shared iPod music library and played through a custom YouTube media backend, so any song you've added to your iPod library is immediately available in Winamp.

Winamp ships with a curated collection of classic skins that can be swapped on the fly, letting you match the player's look to your mood or your current ohmOS theme. Whether you prefer the stock Winamp 2.91 chrome, the Aqua skin for macOS vibes, or the novelty of the Fallout Pip-Boy 2000, the skin system has you covered.
...
*   **Theme-Aware**: Adapts its menu bar placement based on the active ohmOS theme (hidden inside the window frame on XP/Win98 themes).

## User Guide

### Getting Started
Launch Winamp from the ohmOS desktop or application launcher. The player opens with the classic three-panel layout and automatically loads your iPod music library. If you have no tracks yet, it starts with a default track so you can begin listening immediately.
...
### Webamp Integration
The app creates a Webamp instance on mount, rendering it into a positioned container that aligns with the ohmOS window system. Key integration points:
...
*   **Z-Index Management**: The `#webamp` element's z-index is synchronized with ohmOS's foreground state so Winamp layers correctly with other windows.
*   **Close Handling**: Webamp's `onClose` callback is wired to ohmOS's instance close mechanism, ensuring clean teardown.
...
*   **[Soundboard](/docs/soundboard)**: Another audio-focused app in ohmOS for recording and triggering sound effects with keyboard shortcuts.

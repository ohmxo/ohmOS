...
### State Management
`useContactsStore` (`src/stores/useContactsStore.ts`) — Zustand store with IndexedDB persistence (`contacts-storage` in the shared `persisted_state` object store). Holds contacts, selectedContactId, myContactId, and lastRemoteSyncAt. Exposes addContact, updateContact, deleteContact, importContacts, and replaceContactsFromSync. Seed contacts via `seedDefaultContacts`. Cloud sync replaces contacts via `replaceContactsFromSync`.

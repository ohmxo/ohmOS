...
ohmOS uses Zustand for state management with 40 stores following consistent patterns, combined with localStorage and IndexedDB for persistence. Cloud sync (`useCloudSyncStore` + `useAutoCloudSync` + the v2 engine in `src/sync/`) extends persistence to a remote backend: store state is decomposed into per-key documents, deltas travel as journal ops through `/api/sync/v2/*`, and small remote changes apply in real time straight from Pusher events (see `docs/proposals/cloud-sync-v2.md`). CPU-intensive search indexing (Spotlight dynamic results) is offloaded to a dedicated Web Worker rather than running on the main thread.
...
| `useFilesStore` | `ryos:files` (IndexedDB) | Virtual filesystem metadata |
| `useThemeStore` | `ryos:theme` (manual) | OS theme selection + legacy Windows CSS loading |
| `useDisplaySettingsStore` | `ryos:display-settings` | Wallpaper, shaders, screen saver |
| `useDockStore` | `ryos:dock` | Dock pinned items, scale, visibility |
...
| `useCloudSyncStore` | `ryos:cloud-sync` | Cloud sync preferences, per-category status, deletion markers |
...
| Store | Persistence Key | Purpose |
|-------|-----------------|---------|
| `useChatsStore` | `ryos:chats` (IndexedDB) | UI/auth metadata in `persisted_state`; AI messages and room caches in dedicated entity stores |
| `useIpodStore` | `ryos:ipod` (IndexedDB) | Music library, playback state, lyrics |
...
| `useSoundboardStore` | `ryos:soundboard` (IndexedDB) | Board metadata in `persisted_state`; recordings stored as Blob rows in `soundboard_audio` |
| `useAppletStore` | `ryos:applet` | Applet window dimensions |
| `useFinderStore` | `ryos:finder` | Finder instances, view preferences |
| `useTextEditStore` | `ryos:textedit` (IndexedDB) | Scalar metadata in `persisted_state`; open instances and drafts in `textedit_instances` |
...
| `useInfinitePcStore` | `ryos:infinite-pc` | v86 OS-browser presets and runtime state |
| `useStickiesStore` | `ryos:stickies` (IndexedDB) | Sticky notes and note layout |
| `useCalendarStore` | `ryos:calendar` (IndexedDB) | Calendar events, todo items, groups |
| `useContactsStore` | `ryos:contacts` (IndexedDB) | Address book contacts, vCard import/export |
| `useDashboardStore` | `ryos:dashboard` | Dashboard widget layout and configuration |
...
| `useBooksStore` | `ryos:books` | EPUB book library, reading position, last-opened book |
| `useMapsStore` | `ryos:maps:v1` | Maps search results, selected place, map region |
| `useTvStore` | `ryos:tv` | TV channels and playback state |
| `useWeatherStore` | `ryos:weather` | Weather data and saved location |
| `useTimezoneStore` | `ryos:timezone` | World clock timezone selection |
| `useMediaLibraryStore` | (alias of `useIpodStore`) | Shared music-library/lyrics state for iPod, Karaoke, and Winamp |
| `useAirDropStore` | None (ephemeral) | AirDrop transfer session and channel subscription |
| `useWeatherSimulationStore` | None (ephemeral) | Desktop weather-effect simulation state |
| `useAdminDashboardStore` | None (ephemeral) | Admin dashboard UI/query state |
...
Used for large binary data and quota-sensitive Zustand slices:
...
const STORES = {
  ...
  BOOKS: "books",             // EPUB book files
  BOOK_THUMBNAILS: "book_thumbnails", // Generated book cover thumbnails
  ...
  SYNC2_STATE: "sync2_state", // Per-user cursor, shadow, and dirty state
  ...
  VFS_ITEMS: "vfs_items",
  PERSISTED_STATE: "persisted_state", // Scalar metadata / unsplit slices
};

// Database: ohmOS (version 15)
...
Quota-sensitive snapshots use `createIndexedDBPersistStorage()`. Entity-heavy
stores use `createSplitIndexedDBPersistStorage()`, which keeps a small metadata
record in `persisted_state`, migrates monolithic snapshots on hydration, and
writes changed entities into dedicated object stores. Both adapters debounce
and serialize writes and participate in backup flushing. Because IndexedDB
hydration is asynchronous, default seeding and sync jobs wait for
`persist.hasHydrated()` (or the shared `usePersistHydrated` hook).

Cloud Sync v2 keeps its per-user cursor and shadow in the dedicated
`sync2_state` store. This operational metadata migrates from legacy
`ryos:sync2:state:*` localStorage keys and is intentionally excluded from
manual backups.
...
flowchart TB
    subgraph "Metadata Layer"
        META[IndexedDB persisted_state<br/>libraryState]
        FS[IndexedDB vfs_items<br/>path, name, type, uuid, status]
    end
...
hydrate: () => {
  const saved = localStorage.getItem("ryos:theme");
...
```typescript
const USERNAME_RECOVERY_KEY = "ryos:auth:username-recovery";
...
ohmOS now uses typed app-event primitives for most cross-feature interactions:
...
flowchart LR
    ...
    subgraph LargePersist["IndexedDB persisted_state"]
        P2[(ryos:chats)]
        P1[(ryos:ipod)]
        P4[(ryos:textedit)]
    end
...
sequenceDiagram
    ...
    R->>IDB: Load quota-sensitive persisted state
    IDB-->>R: Return state (or migrate legacy localStorage)

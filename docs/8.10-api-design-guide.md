# ohmOS API Design Guide

This document outlines the current patterns and conventions used in the ohmOS API.

## Architecture Overview

The API is implemented as Node.js route handlers under `api/`.  
...
│   ├── storage.ts          # S3 compatible object storage adapter
...
├── sync/                   # Sync v2 journal, blob, preference, and maintenance endpoints
...
### 1. `apiHandler` Wrapper (Primary Pattern)
...
export default apiHandler<z.infer<typeof RequestSchema>>(
  {
    ...
    parseJsonBody: true,      // optional; implied when bodySchema is set
    bodySchema: RequestSchema, // optional Zod validation + typed body
...
### 3. Auth Boundaries via `request-auth`
...
- required auth endpoints need **both** headers:
  ...
  - `X-Username: <username>`
- required endpoints return `400` for partial credentials and `401` for invalid token/username pairs
- optional-auth endpoints allow anonymous requests; partial, stale, or invalid credentials are ignored and resolve to `user: null`
...
- SSE endpoints set stream headers explicitly and emit structured events (no shared SSE module)

## Constants

Use shared constants instead of hardcoding values:

- Runtime-neutral, frontend-shared constants live in `src/shared/` (for example `src/shared/validation.ts` for password/message bounds, `src/shared/redisKeys.ts` for canonical Redis key prefixes, `src/shared/constants/realtime.ts` for realtime channels).
- Auth/session constants (token length, TTLs, grace period, rate-limit windows) live in `api/_utils/auth/_constants.ts`.
- Feature-specific constants stay in feature helper modules (for example `rooms/_helpers/_constants.ts`, `listen/_helpers/_constants.ts`, `songs/_constants.ts`).

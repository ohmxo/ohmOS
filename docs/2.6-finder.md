...
*   **EPUB Books:**
    *   `.epub` files appear with a dedicated "Book" kind and book icon, stored under the protected `/Books` root.
    *   Import a book by dragging or uploading an `.epub` file (it is saved into `/Books`), then double-click it to open in the [Books](/docs/books) reader.
...
### Getting Started
To launch the Finder app, simply click its icon in the ohmOS Dock or locate it within the Applications folder. Upon opening, Finder will typically display your home directory, allowing you to immediately begin browsing and managing your files.
...
*   **Managing Items:**
    ...
    *   **Delete:** Drag files or folders to the `Trash` icon in the Finder window's footer or the ohmOS Dock to send them to the trash.
...
### Tips & Shortcuts
*   **Drag & Drop Efficiency:** Finder fully supports drag-and-drop functionality for moving, copying (if applicable), and deleting files, as well as interacting with other ohmOS applications.
...
-   Default size: 680×400px
-   Minimum size: 300×200px

### Component Architecture
The app consists of 4 component file(s):

-   `src/apps/finder/components/finder-app/FinderAppComponent.tsx`: The primary application component, orchestrating the overall layout, managing global state, and integrating various sub-components.
-   `src/apps/finder/components/file-list/FileList.tsx`: Responsible for rendering the list of files and folders within the current directory, handling user interactions such as selection, drag-and-drop, and displaying file icons.
...
Finder integrates seamlessly with other ohmOS applications; for instance, files can be opened by their respective default applications (e.g., a text file by [TextEdit](/docs/textedit), an `.epub` file by [Books](/docs/books)), and newly installed applications from the [Applet Store](/docs/applet-store) automatically appear in the Applications folder.

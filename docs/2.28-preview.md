...
Preview is ohmOS's lightweight document viewer. It opens images, PDFs, HTML, Markdown, and structured text from Finder, Desktop aliases, drag and drop, or imported device files, then keeps the current document tied to the window so restored sessions reopen the same file.
...
Double-click a supported file in Finder or use a Desktop alias to open it in Preview. If Preview starts empty, click the empty state or choose `File > Import from Device...` to pick a local file and copy it into the ohmOS file system. You can also drag a supported host file onto an open Preview window.

### Key Actions

- **Open from ohmOS**: `File > Open` opens Finder at the root so you can choose a virtual file.

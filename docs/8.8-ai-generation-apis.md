...
ohmOS provides several AI-powered endpoints for content generation, separate from the main chat interface. These endpoints power specialized features like applet generation, time-travel web pages, memory extraction, and music metadata extraction.
...
| 401 | `Unauthorized - invalid token` | Invalid auth token pair when headers are provided |
| 403 | `Unauthorized host` | Host header not in allowlist |
...
Uses `gemini-3-flash-preview` for efficient metadata parsing.
...
These endpoints support ohmOS memory features (daily notes + long-term memory).

### Endpoint Summary

| Method | Path | Description | Auth |
|--------|------|-------------|------|
| POST | `/api/ai/extract-memories` | Extract daily notes + long-term memories from chat conversation | Required |
| POST | `/api/ai/process-daily-notes` | Process unprocessed past daily notes into long-term memories | Required |
...
## Cursor Cloud Agent Endpoints

These endpoints back the chat's Cursor Cloud "repo agent" tool, which runs coding agents asynchronously and streams their events. Both require authentication and are restricted to the admin account (`ryo`); others receive `403`.

### Endpoint Summary

| Method | Path | Description | Auth |
|--------|------|-------------|------|
| GET | `/api/ai/cursor-run-status` | Poll stored events for an async agent run | Admin (`ryo`) |
| POST | `/api/ai/cursor-run-followup` | Send a follow-up prompt to an existing run | Admin (`ryo`) |

### GET /api/ai/cursor-run-status

Polls the Redis-buffered event stream for a Cursor SDK run.

**Query Parameters:**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `runId` | string | Yes | The run to poll |

Returns `404` when the run is unknown and `403` when the run is owned by a different user.
...
`events` are returned in chronological order; `done` is `true` once a `terminal` event is present.

### POST /api/ai/cursor-run-followup

Resumes the Cursor Cloud agent that owns an existing run, sends a new prompt, and starts a fresh background streaming consumer. The new `runId` is returned so the UI can switch its poll target.
...
Both `runId` and `prompt` are required. Returns `503` when `CURSOR_API_KEY` is not configured.
...
- **[API Reference](/docs/api-reference)** - Overview of all ohmOS API endpoints
